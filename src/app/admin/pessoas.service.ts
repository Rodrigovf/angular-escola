...
export class PessoasService {
  ...
  addPessoa(matricula: number, nome: string): Observable<any> {
    const pessoa = {matricula: matricula, nome: nome};
    return this.http.post(this.API_URL + '/pessoas', pessoa);
  }

  updatePessoa(id: number, matricula: number, nome: string): Observable<any> {
    const pessoa = {matricula: matricula, nome: nome};
